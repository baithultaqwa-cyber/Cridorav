/**
 * Metal still in vendor custody: paid customer orders with remaining grams.
 * Includes products no longer listed (hidden / out of stock) — not limited to active SKUs.
 */
export function custodyTotals(rows) {
  ...
  for (const h of list) {
    ...
    exposure += Number(h.buyback_exposure_aed) || 0
      ...
      <table className="w-full text-[11px]">
        <thead className="sticky top-0 z-[1]" style={{ background: 'rgba(20,20,20,0.98)' }}>
          <tr className="text-[#555] text-left">
            ...
            <th className="py-2 px-2 min-w-[120px]">Product</th>
            ...
            <th className="py-2 px-2 whitespace-nowrap">Rate/g</th>
            <th className="py-2 px-2 whitespace-nowrap">Buyback/g</th>
            <th className="py-2 px-2 whitespace-nowrap">Value @ rate</th>
            <th className="py-2 px-2 whitespace-nowrap">Buyback exp.</th>
          </tr>
        </thead>
        <tbody>
          {list.map((h, i) => (
            <tr key={`${idPrefix}-${h.order_id}-${i}`} style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
              ...
              <td className="py-1.5 px-2 text-[#ccc] max-w-[160px]">
              ...
              <td className="py-1.5 px-2 tabular-nums whitespace-nowrap">{Number(h.buyback_per_gram_aed).toFixed(4)}</td>
              ...
              <td className="py-1.5 px-2 tabular-nums whitespace-nowrap text-emerald-400/90">{Number(h.buyback_exposure_aed).toFixed(2)}</td>
            </tr>
          ))}
        </tbody>
        {list.length > 0 && (
          <tfoot>
            <tr style={{ borderTop: '2px solid rgba(201,168,76,0.2)' }}>
              <td colSpan={6} className="py-2 px-2 text-[10px] uppercase tracking-widest text-[#888] font-bold">Totals (custody)</td>
              <td className="py-2 px-2 tabular-nums font-bold text-[#F5F0E8] whitespace-nowrap">{grams.toFixed(4)}</td>
              <td colSpan={2} className="py-2 px-2 text-[#555] text-[10px]">—</td>
